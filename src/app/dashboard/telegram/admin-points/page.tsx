...
export default function TelegramAdminPointsPage() {
  ...
  const fetchStaffStats = async () => {
    setLoading(true);
    // Fetch stats for claimed and closed tickets on Telegram
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4 shrink-0">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black text-zinc-950 tracking-tighter">
            Admin <span className="text-blue-500">Performance</span>
          </h1>
          <p className="text-sm font-bold text-zinc-500 max-w-2xl">
             Monitoring agency operator efficiency and neural response protocols for Telegram sessions.
          </p>
        </div>
      ...
      <div className="flex-1 grid grid-cols-1 xl:grid-cols-12 gap-8 min-h-0 overflow-hidden">
                    ...
                    {loading ? (
                        ...
                        >
                             <div className={`absolute top-6 right-6 w-11 h-11 flex flex-col items-center justify-center rounded-xl shadow-lg transform group-hover:scale-110 transition-transform ${
                                idx === 0 ? 'bg-blue-600 text-white rotate-3' : 'bg-zinc-50 text-zinc-400 border border-zinc-100'
                             }`}>
                             ...
                             <div className="flex items-center gap-5 mb-8">
                                <div className="w-14 h-14 rounded-2xl bg-zinc-50 border border-zinc-100 flex items-center justify-center text-xl font-black text-zinc-300 italic group-hover:bg-blue-600 group-hover:text-white transition-all shadow-inner">
                                    {staff.user_id?.charAt(0) || 'A'}
                                </div>
                                <div className="min-w-0 pr-12">
                                    <h4 className="text-lg font-black text-zinc-950 leading-none mb-1 italic tracking-tighter uppercase truncate underline decoration-zinc-100 underline-offset-4">Admin {cleanId(staff.user_id)}</h4>
                                    <div className="flex items-center gap-2 mt-2">
                                        <div className="w-1.5 h-1.5 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]"></div>
                                        <span className="text-[9px] font-black text-zinc-300 uppercase tracking-widest italic group-hover:text-blue-600 transition-colors">OPERATIONAL_STABLE</span>
                                    </div>
                                </div>
                             </div>
                             ...
                             <div className="grid grid-cols-2 gap-3 mb-6">
                                ...
                                <div className="p-5 bg-zinc-50 rounded-2xl border border-zinc-100 group-hover:bg-blue-600 group-hover:text-white transition-all shadow-inner">
                             ...
                             <div className="space-y-2 px-1">
                                ...
                                <div className="w-full h-1 bg-zinc-50 rounded-full overflow-hidden shadow-inner border border-zinc-100">
                                    <motion.div 
                                        ...
                                        className="h-full bg-blue-600 rounded-full shadow-[0_0_8px_rgba(0,0,0,0.1)]"
                                    ></motion.div>
                                </div>
                             </div>
                        </motion.div>
        ...
        <div className="xl:col-span-4 flex flex-col gap-8 min-h-0">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-8 relative">
                    ...
                    <AnimatePresence mode="popLayout">
                        {staffStats.slice(0, 12).map((staff, idx) => (
                            ...
                            >
                                <div className="absolute left-[-5px] top-1.5 w-2.5 h-2.5 rounded-full bg-white border-2 border-zinc-100 group-hover:border-blue-600 group-hover:bg-blue-600 transition-all z-10 shadow-sm"></div>
             ...
             <div className="p-8 bg-zinc-950 text-white rounded-[2.5rem] shadow-2xl relative overflow-hidden group border border-zinc-900 shrink-0">
                ...
                <button className="w-full py-4 bg-white text-zinc-950 rounded-xl font-black text-[9px] uppercase tracking-[0.3em] shadow-xl hover:scale-[1.02] active:scale-95 transition-all italic underline decoration-blue-500 underline-offset-4">REWARD_ELITE_NODE</button>
            </div>
        </div>
      </div>
    </div>
