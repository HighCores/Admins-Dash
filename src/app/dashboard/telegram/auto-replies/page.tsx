...
import { 
  MessageSquare, Plus, Trash2, Edit3, Save, 
  Loader2, Zap, Sparkles, X, Bot, 
  CheckCircle2, AlertCircle, RefreshCcw, 
  Layout, Monitor, Smartphone, Search,
  ChevronRight, Hash, Send, History, ArrowRight
} from "lucide-react";
import { useState, useEffect } from "react";
import { supabase } from "@/lib/supabase";

export default function TelegramAutoRepliesPage() {
  ...
  const fetchReplies = async () => {
    ...
    const { data } = await supabase
        ...
        .select("*")
        .eq("platform", "telegram") // Ensure we only get Telegram ones if we unify the table
  ...
  const handleEdit = (reply: any) => {
    ...
    setIsActive(reply.is_active ?? true);
  };

  const handleSave = async () => {
    if (!keyword || !response) return alert("All logic nodes must be populated.");
    setSaving(true);
    try {
  ...
  const handleDelete = async (id: any) => {
    if (!confirm("Defragment this logic node?")) return;
    await supabase.from("dc_auto_responses").delete().eq("id", id);
    fetchReplies();
  };
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4 shrink-0">
        <div className="space-y-1">
          <div className="flex items-center gap-3 mb-1">
             ...
             <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest leading-none font-mono">Autonomous Response Relay</span>
          </div>
          <h1 className="text-3xl font-black text-zinc-950 tracking-tighter">
            Auto <span className="text-blue-500">Replies</span>
          </h1>
          <p className="text-sm font-bold text-zinc-500 max-w-2xl">
             Calibrating neural response triggers for Telegram message streams.
          </p>
        </div>
      ...
      <div className="flex-1 grid grid-cols-1 xl:grid-cols-12 gap-8 min-h-0 overflow-hidden">
        ...
        <div className="xl:col-span-8 flex flex-col min-h-0 overflow-hidden">
             <div className="bg-white rounded-[2.5rem] border border-zinc-100 shadow-sm flex-1 flex flex-col overflow-hidden">
                  <div className="grid grid-cols-12 p-6 border-b border-zinc-50 bg-zinc-50/20 text-[9px] font-black text-zinc-400 uppercase tracking-widest">
                      <div className="col-span-4 pl-4">Trigger Logic</div>
                      <div className="col-span-5">Autonomous Output</div>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-2">
                     {loading ? (
                         ...
                         <div className="p-32 text-center opacity-10 italic uppercase font-black tracking-widest font-mono">Neural Interface Silent</div>
                     ) : (
                             ...
                             >
                                 <div className="col-span-4 pl-4">
                                     <div className="flex items-center gap-3">
                                         <div className="p-2 bg-white border border-zinc-100 rounded-lg shadow-sm group-hover:bg-blue-600 group-hover:text-white transition-all">
                                             <Hash size={14} />
                                         </div>
                                         <span className="font-black text-zinc-950 uppercase italic tracking-tighter text-lg leading-none">{reply.keyword}</span>
                                     </div>
                                 </div>
                                 <div className="col-span-5">
                                     <p className="text-xs font-bold text-zinc-500 truncate pr-10 italic">{reply.response_text}</p>
                                 </div>
                                 <div className="col-span-1 text-center">
                                     <div className={`w-2 h-2 rounded-full mx-auto ${reply.is_active ? 'bg-emerald-400 shadow-glow-emerald' : 'bg-red-400 opacity-20'}`}></div>
                                 </div>
        ...
        <div className="xl:col-span-4 flex flex-col gap-8 min-h-0">
             <div className="bg-zinc-950 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group shrink-0">
                <div className="absolute right-0 bottom-0 p-8 opacity-10 rotate-12 group-hover:scale-125 transition-transform duration-1000 pointer-events-none"><Zap size={200} /></div>
                <h3 className="text-xl font-black mb-6 flex items-center gap-4 italic tracking-tighter">
                    <Bot className="text-zinc-400" /> Neural Control
                </h3>
                
                <div className="space-y-4 relative z-10">
                    <div className="flex justify-between items-center bg-white/5 p-5 rounded-2xl border border-white/5">
                        <span className="text-[9px] font-black opacity-30 uppercase italic tracking-widest leading-none">Active Triggers</span>
             ...
             <div className="bg-white p-8 rounded-[3rem] border border-zinc-100 shadow-sm relative overflow-hidden flex-1 flex flex-col min-h-0 group">
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-4 bg-zinc-50 rounded-[2rem] border border-zinc-100 relative overflow-hidden">
                     <div className="flex flex-col items-center gap-6 text-center relative z-10 opacity-20">
                        <Send size={48} className="text-zinc-400 animate-pulse" />
                        <h5 className="text-sm font-black uppercase tracking-[0.2em] italic">Real-time Telemetry Pending</h5>
                     </div>
                </div>
                ...
                <div className="mt-8 text-center shrink-0">
                    <button className="text-[10px] font-black text-zinc-300 uppercase tracking-[0.4em] hover:text-zinc-950 transition-colors flex items-center justify-center gap-3 mx-auto group italic">
                        View Operational Logs <ArrowRight size={14} className="group-hover:translate-x-2 transition-transform" />
                    </button>
                </div>
             </div>
        </div>
      ...
      <AnimatePresence>
        {editingReply && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 backdrop-blur-2xl bg-white/10 animate-in fade-in duration-300">
             ...
             >
                <div className="absolute top-0 right-0 p-12 opacity-5 pointer-events-none rotate-45"><Bot size={240} /></div>
                ...
                <div className="space-y-6 relative z-10">
                    <div className="space-y-2">
                        <label className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.3em] px-4 font-mono leading-none italic">Trigger Keyword (Regex)</label>
                        <input 
                            type="text" 
                    ...
                    <div className="space-y-2">
                        <label className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.3em] px-4 font-mono leading-none italic">Autonomous Output (Response)</label>
                        <textarea 
                            className="w-full p-4 rounded-xl bg-zinc-50 border border-zinc-100 font-bold text-sm text-zinc-950 focus:bg-white outline-none h-32 resize-none transition-all" 
                            value={response}
                            onChange={(e) => setResponse(e.target.value)}
                            placeholder="System response text..."
                        />
                    </div>

                    <div className="flex items-center justify-between p-6 bg-zinc-50 rounded-2xl border border-zinc-100">
                        <div className="flex flex-col">
                            <span className="text-[10px] font-black text-zinc-950 uppercase tracking-widest italic">Operational Status</span>
                            <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest italic">Activate neural node</span>
                        </div>
                        <button 
                            onClick={() => setIsActive(!isActive)}
                            className={`w-14 h-8 rounded-full border-2 transition-all flex items-center ${isActive ? 'bg-zinc-950 border-zinc-950 justify-end' : 'bg-white border-zinc-100 justify-start'}`}
                        >
                            <motion.div 
                                layout
                                className={`w-6 h-6 rounded-full mx-0.5 ${isActive ? 'bg-white' : 'bg-zinc-200'}`}
                            />
                        </button>
                    </div>
                </div>
