...
import { 
  ...
  Hash, Command, Layout, Monitor, Shield, Send
} from "lucide-react";
...
export default function TelegramSetupPage() {
  ...
  const fetchSettings = async () => {
    ...
    if (data) {
        setSettings(data);
        const find = (key: string) => data.find(s => s.key === `tg_${key}`)?.value || "";
  ...
  const handleSave = async () => {
    ...
    try {
        const updates = [
            { key: "tg_log_channel", value: logChannel },
            { key: "tg_welcome_channel", value: welcomeChannel },
            { key: "tg_admin_role", value: adminRole },
            { key: "tg_support_role", value: supportRole },
            { key: "tg_bot_prefix", value: botPrefix },
        ];
        ...
        await supabase.from("dc_stats").insert({
            ...
            details: "Core Telegram nexus settings were recalibrated."
        });

        alert("Telegram Nexus synchronized! 🧠⚡");
    } catch (err: any) {
        alert(err.message);
    } finally {
        setSaving(false);
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4 shrink-0">
        <div className="space-y-1">
          ...
          <p className="text-sm font-bold text-zinc-500 max-w-2xl">
             Calibrating global Telegram settings and monitoring the hub across the network architecture.
          </p>
        </div>
      ...
      <div className="flex-1 grid grid-cols-1 xl:grid-cols-12 gap-8 min-h-0 overflow-hidden">
        ...
        <div className="xl:col-span-8 flex flex-col min-h-0 overflow-hidden">
             <div className="bg-white rounded-[2.5rem] border border-zinc-100 shadow-sm flex-1 flex flex-col overflow-hidden">
                  <div className="p-8 border-b border-zinc-50 bg-zinc-50/20 flex items-center justify-between">
                     ...
                     <span className="bg-blue-600 text-white text-[9px] px-3 py-1.5 rounded-lg font-black tracking-widest leading-none italic">SECURE_TG_STABLE</span>
                  </div>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div className="space-y-10">
                            <div className="space-y-3">
                                <label className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.3em] px-4 font-mono leading-none italic">Incident Log Shard (Chat ID)</label>
                                <input 
                                    type="text"
                                    value={logChannel}
                                    onChange={(e) => setLogChannel(e.target.value)}
                                    className="w-full p-4 rounded-xl bg-zinc-50 border border-zinc-100 font-black text-sm text-zinc-950 outline-none focus:bg-white transition-all shadow-inner"
                                    placeholder="-100xxxxxxx"
                                />
                            </div>
                            <div className="space-y-3">
                                <label className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.3em] px-4 font-mono leading-none italic">Arrival Port (Welcome ID)</label>
                                <input 
                                    type="text"
                        ...
                        <div className="space-y-10">
                            <div className="space-y-3">
                                <label className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.3em] px-4 font-mono leading-none italic">High Authority Key (Admin Role ID)</label>
                                <input 
                                    type="text"
                                    value={adminRole}
                                    onChange={(e) => setAdminRole(e.target.value)}
                                    className="w-full p-4 rounded-xl bg-zinc-50 border border-zinc-100 font-black text-sm text-zinc-950 outline-none focus:bg-white transition-all shadow-inner"
                                    placeholder="Group Admin ID..."
                                />
                            </div>
                            <div className="space-y-3">
                                <label className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.3em] px-4 font-mono leading-none italic">Field Agent Token (Support Role ID)</label>
                                <input 
                                    type="text"
                                    value={supportRole}
                            ...
                            <div className="p-10 bg-zinc-50 border border-zinc-100 rounded-[2.5rem] flex flex-col items-center justify-center text-center group cursor-pointer hover:bg-zinc-950 transition-all relative overflow-hidden h-48 shadow-inner">
                                ...
                                <ShieldCheck size={32} className="text-zinc-300 mb-4 group-hover:text-blue-400 group-hover:scale-125 transition-all" />
        ...
        <div className="xl:col-span-4 flex flex-col gap-8 min-h-0">
             <div className="bg-white p-10 rounded-[3rem] border border-zinc-100 shadow-sm relative overflow-hidden flex flex-col items-center justify-center group flex-1 shrink-0">
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-4 relative w-full">
                     <div className="w-48 h-48 rounded-full border-8 border-zinc-50 relative bg-white shadow-[0_20px_50px_rgba(0,0,0,0.05)] flex items-center justify-center group/pulse">
                          ...
                          <div className="text-center">
                                <div className="text-5xl font-black text-zinc-950 italic tracking-tighter group-hover/pulse:scale-110 transition-transform">100<span className="text-sm opacity-20 ml-1">%</span></div>
                ...
                <div className="w-full space-y-4 mt-8">
                     <div className="flex items-center justify-between border-b border-zinc-50 pb-4">
                        ...
                        <span className="text-lg font-black text-zinc-950 italic tracking-tighter uppercase">TG_SHARD_01</span>
                     </div>
                     <div className="flex items-center justify-between border-b border-zinc-50 pb-4">
                        <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest italic">Sync Latency</span>
                        <span className="text-lg font-black text-blue-600 italic tracking-tighter uppercase">0.08ms_Optimal</span>
                     </div>
                </div>
             </div>
             ...
             <div className="p-8 bg-zinc-950 text-white rounded-[2.5rem] shadow-2xl relative overflow-hidden group border border-zinc-900 shrink-0">
                ...
                <p className="text-[10px] opacity-40 mb-6 font-bold leading-relaxed pr-10 italic">Emergency security failsafe. Abort all active entity threads immediately across all network shards.</p>
                <button className="w-full py-4 bg-white text-zinc-950 rounded-xl font-black text-[9px] uppercase tracking-[0.3em] shadow-xl hover:bg-zinc-100 transition-all italic underline decoration-blue-200 underline-offset-4">TERMINATE_ENTITY_CYCLE</button>
            </div>
        </div>
      </div>
    </div>
