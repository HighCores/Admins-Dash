...
const Navigation = [
  ...
  { name: "Panels", icon: PanelsTopLeft, href: "/dashboard/discord/panels" },
  { name: "Commands", icon: Command, href: "/dashboard/discord/commands" },
  { name: "Auto Replies", icon: MessageSquare, href: "/dashboard/discord/auto-replies" },
  { name: "Levels & Roles", icon: TrendingUp, href: "/dashboard/discord/levels" },
  { name: "General Points", icon: Coins, href: "/dashboard/discord/points" },
...
export default function Sidebar() {
  ...
  return (
    <div className="flex flex-col h-full bg-white text-zinc-900 border-r border-zinc-100 p-6 overflow-hidden shadow-[1px_0_0_0_rgba(0,0,0,0.02)]">
      ...
      <div className="mb-10 mx-2 p-1 bg-zinc-100 rounded-2xl flex relative h-14 items-center shrink-0">
        <motion.div
      ...
      <nav className="flex-1 space-y-1 custom-scrollbar overflow-y-auto px-2">
        <div className="text-[9px] font-black text-zinc-300 uppercase tracking-[0.3em] mb-4 pl-4 font-mono">Operations Fleet</div>
        ...
        {Navigation.map((item) => {
            ...
            return (
                <Link key={item.name} href={finalHref}>
                    <motion.div 
                        whileHover={{ x: 5 }}
                        className={`group flex items-center justify-between p-3.5 rounded-xl transition-all mb-1 ${
                            isActive 
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <span className={`text-sm font-bold tracking-tight ${isActive ? "font-black text-zinc-950" : "font-semibold"}`}>
                                {item.name}
                            </span>
                        </div>
                        {isActive && (
      ...
      {/* Footer / User Profile */}
      <div className="mt-8 pt-6 border-t border-zinc-100 px-2 shrink-0">
